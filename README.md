...
Яндекс Диск широко используется для обмена записями конференций, лекциями, онлайн-курсами и другими файлами. Но часто авторы полностью запрещают скачивание — нет кнопки скачать, нет возможности сохранить файл, нет шансов получить данные.

**yadisk-downloader** решает эту проблему: вставьте публичную ссылку, и инструмент скачает все файлы за один раз.

### Как это работает?

- **Видео** — скачиваются через HLS-потоки (intercept m3u8 → ffmpeg) в оригинальном качестве
- **Документы, фото и другие файлы** — когда владелец отключил скачивание, инструмент открывает файл в Яндекс Документах (docviewer) и сохраняет отрендеренное содержимое как PDF
- **Обычные ссылки** — скачиваются напрямую через API Яндекс Диска

> **Примечание:** Если владелец отключил скачивание, документы и изображения сохраняются в формате PDF (всплывающее предупреждение об этом). Оригинальный формат недоступен из-за ограничений сервера Яндекс Диска.

## Возможности

- **Скачивание с отключённым скачиванием** — работает даже когда владелец запретил скачивание
...
- **HLS-скачивание видео** — intercept m3u8 → ffmpeg, лучшее качество
- **Docviewer fallback** — рендеринг документов/фото через Яндекс Документы
...
- **Уведомления** — системные уведомления + всплывающие окна предупреждений
